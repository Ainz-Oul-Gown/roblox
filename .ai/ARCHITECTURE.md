...
- `CharacterAnimator` (клиентский движок процедурной анимации):
  ...
  - Поддержка R6 сустава `RootJoint`: выверенная кинематика базиса, где вращение торса вокруг вертикальной оси (Yaw) выполняется вокруг локальной оси Z (`CFrame.Angles(0, 0, angle)`), полностью исключая переворачивание R6-персонажей в пол.
  - Двухфазный 360° поворот в `playSpinSalute` с интерполяцией от 180° через -179° к 0°, исключающий схлопывание кватернионов в противоположную сторону.
  ...
  - Автоматическое кэширование начального положения суставов (`originalC0Cache`) со слабыми ссылками (`__mode = "k"`), предотвращающими утечки памяти при смерти и респавне персонажей. Гарантированный сброс `restoreJoints` по истечении длительности или при новом вызове.
...
- `CinematicCamera` (клиентский режиссер кинематографической камеры):
  - Модуль драматического управления перспективой камеры во время активации ключевых способностей:
    * `focusCutIn`: кинематографический наезд перед лицом/грудью персонажа с настраиваемым голландским углом (Dutch Angle: наклон горизонта на 10-15 градусов) и компрессией FOV (70 -> 52). Защита от FOV Drift: `originalFOV` фиксируется только если кинематографический режим не был активен ранее.
    * Защита от клиппинга и проникновения в стены: быстрый `Workspace:Raycast` вдоль луча наезда, ограничивающий приближение камеры при наличии стен или интерьеров тайкуна.
    * `groundSlamPerspective`: драматический нижний ракурс снизу вверх для тяжелых приземлений и падений объектов с неба с проверкой уровня пола через `Workspace:Raycast` (камера не проваливается под плиту перекрытия или фундамент).
    * `explosiveSnapBack`: мгновенный отскок камеры с импульсом FOV и тряской при ударе.
    * Защищенный сброс (`resetCamera` / `safeReset`): обязательная отмена активных твинов камеры `activeCameraTween:Cancel()` (исключает борьбу твинов за `FieldOfView`), гарантированное возвращение `CameraType.Custom` и `CameraSubject` к Humanoid при дисконнекте, смерти (`Humanoid.Died`) или респавне (`localPlayer.CharacterAdded`).
