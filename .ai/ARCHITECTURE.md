...
- `PlotBuilder`: Генерация баз, конвейеров, апгрейдеров, портала Rebirth и монументов:
  ...
  - **Безопасные кнопки покупок**: Кнопки способностей и апгрейдов разнесены по широким залам крыльев ($X=\pm 20$); лестничный марш и лестничная площадка свободны от кнопок на 100%.
- `AbilityService`: Инвентарные тулы способностей, кулдауны, PvP-урон, серверная репликация эффектов на всех клиентов через `AbilityVFXEvent:FireAllClients`.
