...
- `CharacterAnimator` (клиентский движок процедурной анимации):
  ...
  - Централизованное управление твинами суставов: таблица со слабыми ссылками `activeJointTweens` и функция `tweenJoint` с обязательной отменой (`:Cancel()`) предыдущих активных твинов на суставе перед запуском новых, полностью исключая дерганье и рассинхрон конечностей.
...
- `CinematicCamera` (клиентский режиссер кинематографической камеры):
  - Модуль драматического управления перспективой камеры во время активации ключевых способностей:
    * `focusCutIn`: кинематографический наезд перед лицом/грудью персонажа с настраиваемым голландским углом (Dutch Angle: наклон горизонта на 10-15 градусов) и компрессией FOV.
    ...
    * `groundSlamPerspective`: драматический нижний ракурс снизу вверх для тяжелых приземлений и падений объектов с неба с проверкой уровня пола (0, -6, 0) и препятствий позади персонажа (`-root.CFrame.LookVector * desiredDist`), предотвращающий пробитие стен арены и тайкуна сзади.
    * `explosiveSnapBack`: мгновенный отскок камеры с импульсом FOV и экраном тряски в момент импакта ультимейтов.
    * Сохранение кастомного FOV игрока: `originalFOV` фиксируется только на время синематика и не перезаписывает пользовательский угол обзора при спавне персонажа.
    * Защищенный сброс (`resetCamera` / `safeReset`): обязательная отмена активных твинов камеры `activeCameraTween:Cancel()`, гарантированное возвращение `CameraType.Custom` и `CameraSubject` к Humanoid при дисконнекте, смерти (`Humanoid.Died`) или респавне (`localPlayer.CharacterAdded`).
