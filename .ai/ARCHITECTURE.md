...
- `CharacterAnimator` (клиентский движок процедурной анимации):
  ...
  - Управление суставами `Motor6D` (`RightShoulder`, `LeftShoulder`, `Waist/RootJoint`, `Neck`) через `TweenService` с динамической интерполяцией C0 CFrame (поддержка `joints.waist or joints.rootJoint` для полноценной анимации торса на R6).
...
- `CinematicCamera` (клиентский режиссер кинематографической камеры):
  - Модуль драматического управления перспективой камеры во время активации ключевых способностей:
    ...
    * Защищенный сброс (`resetCamera` / `safeReset`): гарантированное возвращение `CameraType.Custom` и `CameraSubject` к Humanoid даже при дисконнекте, смерти или спаме абилок, включая автоматический слушатель `localPlayer.CharacterAdded`.
