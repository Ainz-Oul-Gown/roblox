...
- **SFX & Clean Audio Design**: Чистые приятные звуки с нормализацией громкости: легендарный кассовый аппарат «Ча-чинг!» (`cash` / `snatch`: `rbxassetid://131070686`, комфортная громкость 0.45 без фальшивого детюнинга), кристальный щелчок покупки (`buy`: `rbxassetid://6895079853`), и боевые звуки (`slam`, `laser`, `dash`, `parry` и др.) со случайной вариацией высоты тона (`PlaybackSpeed 0.94..1.06`) для устранения монотонности в бою.
...
- `MonetizationService`: Промышленная обработка чеков `MarketplaceService.ProcessReceipt` с двойной идемпотентностью (RAM + DataStore `ProcessedReceipts_v1`), кэшированием геймпасов (VIP, Double Cash, Auto Collector, Hyper Speed), автоматическим перехватом `PromptGamePassPurchaseFinished` для моментальной активации перков прямо в игре, и полнофункциональным клиентским Кибер-Магазином (`ShopModal` с переключением табов геймпасов и продуктов, кнопкой в HUD и хоткеем `[B]`).
...
- `CharacterAnimator` (клиентский движок процедурной анимации):
  ...
  - Обход ограничения движка Roblox на read-only `Motor6D.C0`: в Roblox C++ архитектуре реплицируемое свойство `JointInstance.C0` на клиенте помечено как ReadOnly. `CharacterAnimator` использует надежный двухконтурный механизм: прокси `CFrameValue` для плавного расчета кривых интерполяции `TweenService`, безопасное присваивание `pcall(function() joint.C0 = newVal end)` и применение относительного поворота к нативному свойству `Motor6D.Transform` в цикле `RunService.Stepped` / `PreSimulation`. Это гарантирует безупречную процедурную анимацию без ошибок движка и без конфликта с базовыми анимациями бега.
  - Корректная кинематика R6 и R15:
    ...
    * Вращение вокруг вертикальной оси (Yaw в `playSpinSalute`) выполняется по локальной оси Z (`CFrame.Angles(0, 0, math.rad(180))`) как для R15, так и для R6, исключая выворачивание персонажа наизнанку.
  - Потокобезопасный переход фаз в `playSpinSalute`: переход на обратную фазу через токенизированный `task.delay` без блокирующего ожидания `tw.Completed:Wait()`, что гарантирует отсутствие зависших мертвых корутин при прерывании анимации.
  - Очистка связей жизненного цикла: безопасное отключение `deadConnections[hum]:Disconnect()` и очистка ссылки при срабатывании `Humanoid.Died`.
  ...
  - Набор из 9 сигнатурных поз мемов поколения Альфа:
    * `playShhhPose`: легендарный жест тишины (поднятие пальца к губам, наклон головы и выдвижение челюсти) с разворотом торса на 22° в 3/4 профиль, делающим позу отчетливо видимой со спины при стандартном виде от 3-го лица.
    * `playGigachadFlex`: двойной бицепс-флекс (поднятие и сгибание обоих предплечий с наклоном груди назад).
    * `playSigmaTilt`: фирменный холодный наклон головы вбок и легкий разворот торса на 16° в полупрофиль для четкой видимости со спины.
    ...
    * `playRizzlerFlourish`: манерный жест руки у сердца с изящным сгибом локтя (75°), наклоном головы и полуразворотом корпуса на 18° (без вытянутой руки-салюта).
    * `playHypnoHands`: гипнотический жест разведенных кистей с легким покачиванием для тактических ментальных атак.
    * `playArcherDraw`: натяжение тетивы амура с отведением локтя назад и прицеливанием вперед.
    * `playGentlemanBow`: благородный аристократический поклон с прижатой к животу рукой и наклоном корпуса вперед.
    * `playSpinSalute`: вихревой поворот вокруг вертикальной оси с армейским салютом.
    * `playMoneySnatch`: выпад двумя руками вперед для захвата наличных с наклоном торса вперед на 18° и откатом назад к груди.
    * `playDashPose`: хлесткий динамический рывок длительностью всего 0.22с с аккуратным наклоном торса вперед на 10° (без заклинивания корпуса в согнутом положении), приподнятой головой и вариантами позы (аэродинамический спринт с отведенными назад руками либо выставленный вперед таран плечом `isShoulderCharge`).
  - Робастный поиск суставов `findMotor6D`: поддержка различных родителей (`Torso`, `HumanoidRootPart`, `UpperTorso`, `LowerTorso`) и названий конечностей (`RightUpperArm`/`RightArm`, `LeftUpperArm`/`LeftArm`), гарантирующая работу на любых пользовательских аватарах и бандлах.
- `CinematicCamera` (клиентский режиссер кинематографической камеры):
  - Модуль драматического управления перспективой камеры во время активации ключевых способностей:
    ...
    * Динамическая защита от клиппинга и окклюзии: рейкасты коллизий исключают всех игроков сервера (`Players:GetPlayers()`), минимальная дистанция отсечения ограничена 1.8 стада (исключает клиппинг камеры внутрь полигонов черепа); в цикле `RenderStepped` выполняется проверка окклюзии в реальном времени (`dynRay`), предотвращающая пробитие стен при движении или отбрасывании.
    * `groundSlamPerspective`: драматический нижний ракурс снизу вверх с непрерывным отслеживанием перемещений персонажа в `RenderStepped`, динамической проверкой уровня пола в реальном времени (`curFloorHit`), предотвращающей проваливание камеры под землю при падениях с высоты.
    * `explosiveSnapBack`: мгновенный отскок камеры с импульсом FOV и экраном тряски в момент импакта ультимейтов.
    * Изоляция от FOV Drift: `originalFOV` жестко привязан к базовому значению (`DEFAULT_BASE_FOV = 70`), исключая захват искаженных промежуточных значений от сторонних `fovPulse`.
    * Защищенный сброс (`resetCamera` / `safeReset`): автоматическая установка CFrame камеры со спины персонажа (`root.Position - lookVec * 12.5 + Vector3.new(0, 3.2, 0)`) перед возвратом `CameraType.Custom`, гарантирующая что стандартный контроллер камеры Roblox не застрянет направленным в лицо игрока; отключение соединения `cameraUpdateConnection:Disconnect()`, безопасная отмена `activeCameraThread` с защитой от самоотмены текущей корутины (`threadToCancel ~= coroutine.running()`) и `pcall` (устранение рантайм ошибки Studio `cannot cancel thread`), обязательная отмена и удаление твинов камеры `activeCameraTween:Cancel()` + `activeCameraTween:Destroy()`, гарантированное возвращение `CameraType.Custom` и `CameraSubject` к Humanoid при дисконнекте, смерти или респавне (`localPlayer.CharacterAdded`).
- `JuiceEffects` (клиентский банк сочности и тактильного отклика):
  - Безопасное управление потоками тряски камеры `activeShakeThread` с защитой от самоотмены и полным отсутствием утечек потоков.
  - Полный аудит аудио-ассетов: устранены ошибки "Asset type does not match requested type" и случайные звуки выстрелов при сборе валюты. Все звуки монет и покупки переведены на кристальный колокольчик (`rbxassetid://6895079853`), а взрывы, рывки и брызги подключены к нативным встроенным клиенту ассетам `rbxasset://sounds/...`, гарантируя 100% стабильность воспроизведения без сетевых сбоев.
- `AbilityVFX` (клиентский движок эффектов):
  - 48 уникальных наборов визуальных и звуковых эффектов (8 фракций x 6 слотов способностей), реализованных по 4-фазной модели (Anticipation -> Release -> World Fracture -> Dissipation). 100% покрытие способностей процедурными анимациями `CharacterAnimator` (включая все tactical слоты).
  ...
  - Клиентская физика рывков `performDashImpulse`: создание временного констрейнта `LinearVelocity` (`MaxForce = 1e6`, длительность ~0.28с) в связке с начальным всплеском `AssemblyLinearVelocity`. Гарантирует преодоление трения пола и мгновенный реальный физический рывок локального персонажа вперед на 25-30 стадов со скоростью 80-100 стад/с независимо от сетевой задержки.
  - Мгновенный клиентский телепорт `TELEPORT`: безопасная маршрутизация `abilityVFXEvent:FireClient(player, "TELEPORT", blinkCF)` с проверкой типа `typeof(targetCFrame) == "CFrame"` и прямым вызовом `myChar:PivotTo(targetCFrame)` на клиенте, исключающая конфликт сетевого владения для Sigma (`ShadowStep`) и Rizzler (`HeartbreakDash`).
  - Локальная ориентация защитного барьера парирования: в слоте `Sigma` tactical барьер выстраивается строго в направлении взгляда персонажа с использованием `sideVec = lookVector:Cross(Vector3.yAxis)` и рунического круга `spawnMagicCircle`, исключая пересечение с телом кастера по мировым осям.
  - Полное устранение статичных лазерных цилиндров из всех 8 слотов мобильности (`Skibidi`, `Mewing`, `Sigma`, `FanumTax`, `Grimace`, `CaseOh`, `Rizzler`, `TungTung`), замененных на процедурный рывок `CharacterAnimator.playDashPose`, динамические ударные кольца и свистящий аэродинамический звук.
  - LOD-оптимизация процедурных анимаций: проверка `shouldAnimate = rawCasterChar ~= nil and (isSelf or isWithinLOD(pos))` отсекает создание избыточных твинов суставов для персонажей вне радиуса обзора 120 стадов, полностью устраняя риск C++ DoS-зависаний при сетевом спаме способностей.
