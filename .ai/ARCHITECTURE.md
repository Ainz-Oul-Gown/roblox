...
- `CharacterAnimator` (клиентский движок процедурной анимации):
  ...
  - Обход ограничения движка Roblox на read-only `Motor6D.C0`: в Roblox C++ метаданных `TweenService:Create` запрещает твинить `C0` напрямую (`property named 'C0' on object 'Motor6D' is read only`). `tweenJoint` использует легковесный прокси `CFrameValue`, твиня его свойство `Value` через TweenService и обновляя `joint.C0 = newVal` в событии `Changed`, с очисткой соединения и объекта в `Destroying`/`Completed`.
