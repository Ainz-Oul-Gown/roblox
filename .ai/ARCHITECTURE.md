...
- **2-й этаж**:
  - `Floor2_Foundation`: сплошное перекрытие $56 \times 76$ ровно на высоте стен $Y = 14$ (без щелей и зависаний в воздухе) с вырезом под лестничный пролет.
  ...
  - `Rebirth_Portal`: физический золотой алтарь перерождения на 2 этаже. Вход в поле выполняет `TycoonService.performRebirth`, оповещает через `RebirthEvent` с фанфарами и вспышкой, сбрасывает базу и сохраняет повышенный множитель и ранг.
