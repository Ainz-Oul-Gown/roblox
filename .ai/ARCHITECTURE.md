...
- `CharacterAnimator` (клиентский движок процедурной анимации):
  ...
  - Централизованное управление твинами суставов: таблица со слабыми ссылками `activeJointTweens` и функция `tweenJoint` с обязательной отменой (`:Cancel()`) предыдущих активных твинов на суставе и автоматической очисткой ссылок по завершению (`tw.Completed`), исключая дерганье и утечки памяти.
  - Валидация живого состояния `isCharacterAlive(character)`: все отложенные фазы ударов и возврата суставов проверяют `Humanoid.Health > 0`, предотвращая деформацию и поломку физики трупов при гибели игрока во время каста.
  ...
  - Поддержка R6 и R15 сустава `rootJoint`: вращение всего аватара в `playSpinSalute` вокруг вертикальной оси (LowerTorso.Root в R15 и HumanoidRootPart.RootJoint в R6) без скручивания позвоночника в Waist.
...
- `CinematicCamera` (клиентский режиссер кинематографической камеры):
  - Модуль драматического управления перспективой камеры во время активации ключевых способностей:
    * `focusCutIn`: кинематографический наезд перед лицом/грудью персонажа с настраиваемым голландским углом (Dutch Angle: наклон горизонта на 10-15 градусов), компрессией FOV и динамическим трекингом персонажа в `RunService.RenderStepped` (предотвращает отставание камеры и клиппинг при движении на высокой скорости).
    * Защита от клиппинга и проникновения в стены спереди: `Workspace:Raycast` вдоль направления наезда с динамическим отступом (0.4 studs buffer), предотвращающий выход камеры за пределы стен.
    * `groundSlamPerspective`: драматический нижний ракурс снизу вверх с непрерывным отслеживанием перемещений персонажа в `RenderStepped`, проверкой уровня пола (0, -6, 0) и препятствий позади персонажа (`-root.CFrame.LookVector * desiredDist`), предотвращающий пробитие стен арены и тайкуна сзади.
    * `explosiveSnapBack`: мгновенный отскок камеры с импульсом FOV и экраном тряски в момент импакта ультимейтов.
    * Сохранение кастомного FOV игрока: `originalFOV` фиксируется только на время синематика и нормализуется через `math.clamp(originalFOV, 65, 75)`, предотвращая дрейф угла обзора при наложении внешних FOV-эффектов.
    * Защищенный сброс (`resetCamera` / `safeReset`): отключение соединения `cameraUpdateConnection:Disconnect()`, обязательная отмена активных твинов камеры `activeCameraTween:Cancel()`, гарантированное возвращение `CameraType.Custom` и `CameraSubject` к Humanoid при дисконнекте, смерти (`Humanoid.Died` с отложенным захватом через `task.spawn` + `WaitForChild`) или респавне (`localPlayer.CharacterAdded`).
