...
## 4. Мультиагентный регламент (.agents/rules/multi_agent_pipeline.md)
Все задачи по разработке кода проходят 4-ролевой конвейер:
...
4. **Код-ревьюер / QA**: проверка соответствия Selene/StyLua, запуск `npm test` до `[CODE_APPROVED]`.
